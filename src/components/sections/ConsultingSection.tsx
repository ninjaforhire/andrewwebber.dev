...
function GraphicIntro() {
  return (
    <svg viewBox="0 0 200 100" className="w-full h-full" aria-hidden="true">
      <defs>
        <radialGradient id="cg-ig1" cx="35%" cy="50%" r="50%">
          <stop offset="0%" stopColor="#a78bfa" stopOpacity="0.6" />
          <stop offset="100%" stopColor="#a78bfa" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="cg-ig2" cx="65%" cy="50%" r="50%">
      ...
      <circle cx="68" cy="50" r="36" fill="url(#cg-ig1)" />
      <circle cx="132" cy="50" r="36" fill="url(#cg-ig2)" />
      ...
      <line x1="90" y1="50" x2="110" y2="50" stroke="#c4b5fd" strokeWidth="1.5" strokeDasharray="3 2" opacity="0.6" filter="url(#cg-iglow)" />
      <circle cx="68" cy="50" r="5" fill="#a78bfa" opacity="0.9" filter="url(#cg-iglow)" />
      <circle cx="132" cy="50" r="5" fill="#7c3aed" opacity="0.9" filter="url(#cg-iglow)" />
...
function GraphicOwner() {
  ...
  const dots = Array.from({ length: rows * cols }, (_, i) => ({
    x: 24 + (i % cols) * 24,
    y: 20 + Math.floor(i / cols) * 20,
    i,
  }));
  return (
    <svg viewBox="0 0 200 100" className="w-full h-full" aria-hidden="true">
      <defs>
        <radialGradient id="cg-og" cx="50%" cy="50%" r="60%">
      ...
      <rect x="0" y="0" width="200" height="100" fill="url(#cg-og)" />
      ...
      {dots.map((d) => (
        <circle key={d.i} cx={d.x} cy={d.y}
          ...
          filter={highlightPath.has(d.i) ? "url(#cg-oglow)" : undefined}
        />
      ))}
...
function GraphicDiscovery() {
  return (
    <svg viewBox="0 0 200 100" className="w-full h-full" aria-hidden="true">
      <defs>
        <radialGradient id="cg-dg" cx="45%" cy="55%" r="55%">
        ...
        <clipPath id="cg-dclip"><circle cx="82" cy="52" r="42" /></clipPath>
      </defs>
      <circle cx="82" cy="52" r="42" fill="url(#cg-dg)" />
      {[14, 24, 34, 42].map((r, i) => (
        <circle key={r} cx="82" cy="52" r={r} fill="none" stroke="#34d399" strokeWidth="0.8"
          opacity={0.5 - i * 0.1} strokeDasharray={i === 3 ? "none" : "3 2"} />
      ))}
      <line x1="82" y1="12" x2="82" y2="92" stroke="#34d399" strokeWidth="0.6" opacity="0.25" clipPath="url(#cg-dclip)" />
      <line x1="42" y1="52" x2="122" y2="52" stroke="#34d399" strokeWidth="0.6" opacity="0.25" clipPath="url(#cg-dclip)" />
      <line x1="82" y1="52" x2="115" y2="24" stroke="#34d399" strokeWidth="1.2" opacity="0.7" filter="url(#cg-dglow)" />
      <circle cx="82" cy="52" r="3.5" fill="#34d399" opacity="0.9" filter="url(#cg-dglow)" />
      <circle cx="111" cy="29" r="2.5" fill="#6ee7b7" opacity="0.8" filter="url(#cg-dglow)" />
...
function GraphicImplementation() {
  ...
  return (
    <svg viewBox="0 0 200 100" className="w-full h-full" aria-hidden="true">
      <defs>
        <radialGradient id="cg-amg" cx="50%" cy="50%" r="55%">
        ...
        <linearGradient id="cg-ambar" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="#f59e0b" stopOpacity="0.6" />
          <stop offset="100%" stopColor="#fbbf24" stopOpacity="0.2" />
        </linearGradient>
      </defs>
      <rect x="0" y="0" width="200" height="100" fill="url(#cg-amg)" />
      {layers.map((l, i) => (
        <g key={l.label}>
          <rect x={55 - i * 5} y={l.y} width={l.w} height="14" rx="3" fill="url(#cg-ambar)" opacity={0.9 - i * 0.15} />
      ...
      <circle cx="95" cy="88" r="3" fill="#fbbf24" opacity="0.9" filter="url(#cg-amglow)" />
...
function ArchitectureGraphic() {
  ...
  const edges = [[0,1],[1,2],[1,3],[2,4],[3,4],[4,5],[3,6],[3,7],[4,8],[6,7],[7,8],[2,5]] as [number,number][];
  return (
    <svg viewBox="0 0 420 230" className="w-full h-full opacity-70" aria-hidden="true">
      <defs>
        <radialGradient id="cg-rng" cx="50%" cy="50%" r="50%">
        ...
        <linearGradient id="cg-redge" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      {edges.map(([a, b], i) => (
        <line key={i} x1={nodes[a].cx} y1={nodes[a].cy} x2={nodes[b].cx} y2={nodes[b].cy}
          stroke="url(#cg-redge)" strokeWidth="1" />
      ))}
      {nodes.map((n, i) => (
        <circle key={`h${i}`} cx={n.cx} cy={n.cy} r={n.r * 3} fill="url(#cg-rng)" opacity="0.2" />
      ))}
      {nodes.map((n, i) => (
...
const ACCENT = {
  purple: {
    label: "text-violet-400", border: "border-violet-500/30",
    gradFrom: "rgba(76,29,149,0.25)", glowColor: "#7c3aed",
  ...
  blue: {
    label: "text-sky-400", border: "border-sky-500/30",
    gradFrom: "rgba(12,74,110,0.28)", glowColor: "#0ea5e9",
    check: "text-sky-400",
  ...
  emerald: {
    label: "text-emerald-400", border: "border-emerald-500/30",
    gradFrom: "rgba(6,78,59,0.28)", glowColor: "#10b981",
  ...
  amber: {
    label: "text-amber-400", border: "border-amber-500/30",
    gradFrom: "rgba(120,53,15,0.28)", glowColor: "#f59e0b",
...
function ConsultingCard({
  label, price, sub, desc, features, accent, featured,
  cta = "Book Now →", badge, note, graphic,
}: ConsultingCardProps) {
  ...
  return (
    <div
      ...
      style={{ background: `linear-gradient(160deg, ${c.gradFrom} 0%, rgba(13,13,15,0) 55%, rgba(13,13,15,0.95) 100%)` }}
    >
      ...
      <div className="relative flex flex-col flex-1 p-7">
        ...
        <a href="#consultation"
          className={cn(
            "mt-5 block rounded-md py-2.5 text-center font-mono text-xs uppercase tracking-wider border transition-colors",
